...
func RegisterRoutes(cliCtx context.CLIContext, r *mux.Router) {
	r.HandleFunc(fmt.Sprintf("/ouroboros/profile/{%s}", restName), getHandler(cliCtx)).Methods("GET")
...
func getHandler(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/ouroboros/profile/%s", paramType), nil)
